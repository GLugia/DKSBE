...
{
	public unsafe class Unk_4F : ITableObject
	{
		public List<Unk_4F_Data?>? data;
		...
		public Dictionary<long, PtrData?>? Read(byte* origin, ref long offset)
		{
			...
			for (int i = 0; i < ptrs.Length; i++)
			{
				...
				{
					this.data.Add((Unk_4F_Data?)Stagebase.current_ptr_data?[offset]?.value);
					continue;
				}
				if (result.ContainsKey(offset))
				{
					this.data.Add((Unk_4F_Data?)result[offset]?.value);
					continue;
				}
				Unk_4F_Data value = new();
